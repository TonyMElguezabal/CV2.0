...
const contentPath = (...segments: string[]) =>
  resolve(__dirname, "..", "..", "content", ...segments);

describe("content-model: profile", () => {
  it("parses content/profile.yaml into the expected Profile shape", () => {
    const raw = readFileSync(contentPath("profile.yaml"), "utf-8");
...
describe("content-model: experience", () => {
  it("parses experience/acme-corp.yaml into the expected Experience shape, with chapter ID from its filename", () => {
    const path = contentPath("experience", "acme-corp.yaml");
    ...
    expect(chapterId).toBe("acme-corp");
  });
});

describe("content-model: project", () => {
  it("parses projects/dashboard-revamp.md frontmatter and body into the expected Project shape, with project ID from its filename", () => {
    const path = contentPath("projects", "dashboard-revamp.md");
    const raw = readFileSync(path, "utf-8");
    ...
    expect(projectId).toBe("dashboard-revamp");
  });
});

describe("content-model: skills", () => {
  it("resolves every skills.yaml evidence reference to an existing experience or project slug", () => {
    const raw = readFileSync(contentPath("skills.yaml"), "utf-8");
    const skills = parseYaml(raw) as Skill[];

    expect(skills.length).toBeGreaterThan(0);

    const experienceSlugs = readdirSync(contentPath("experience")).map((f) =>
      basename(f, extname(f)),
    );
    const projectSlugs = readdirSync(contentPath("projects")).map((f) =>
      basename(f, extname(f)),
    );
...
describe("content-model: faq", () => {
  it("contains at least one parsable question/answer pair", () => {
    const raw = readFileSync(contentPath("faq.md"), "utf-8");
...
describe("content-model: file structure", () => {
  it("has every content type present as a structured file, separate from any UI component", () => {
    expect(existsSync(contentPath("profile.yaml"))).toBe(true);
    expect(readdirSync(contentPath("experience")).length).toBeGreaterThan(0);
    expect(readdirSync(contentPath("projects")).length).toBeGreaterThan(0);
    expect(existsSync(contentPath("skills.yaml"))).toBe(true);
    expect(existsSync(contentPath("faq.md"))).toBe(true);
  });
});
