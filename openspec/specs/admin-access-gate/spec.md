...
Owner-only access control for the admin area. Gates `/admin` behind HTTP
Basic Auth so the site owner can reach the insights dashboard while every
other request is denied, with no users table and no cookie on the public
site.
