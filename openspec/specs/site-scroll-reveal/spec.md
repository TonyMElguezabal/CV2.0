...
Defines scroll-triggered content reveals: headings that resolve from a
blur-up entrance while rising into place, and section-level entrances that
fade and rise as content enters the viewport. This capability exists because
the page below the hero previously had no motion at all — content simply
appeared, fully formed. The reveal treatment is deliberately scoped to
headings and section entrances only; career chapter body text, dates, role
descriptions, metrics, and skill evidence links are never individually
gated, so the page stays legible to scanning and to in-page find at any
time. The animation is constructed to animate only `opacity` and `transform`
(any blur is a static, pre-rendered effect that is cross-faded rather than
animated per frame), and the mechanism is a progressive enhancement over
already-readable content: a reveal that never fires — an unfired observer,
a scripting error, an unsupported browser, or JavaScript disabled — must
