# chat-guardrails-and-cost-controls

## Purpose

Defines the abuse-prevention and cost-control guardrails around the chat
endpoint: per-IP and per-session request limits, a contactable fallback for
rejected visitors, a cap on generated output length, and testability of the
rate-limit decision logic without a live store.
