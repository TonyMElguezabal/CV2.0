...
Defines the site-wide security & privacy guarantees not owned by any single feature capability — secrets are server-side only, every response carries a CSP and hardening headers, the public AI endpoint validates input, and chat conversations are never persisted server-side.
