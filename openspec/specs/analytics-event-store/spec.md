...
Defines the first-party analytics persistence layer: the anonymized `VisitSession`/`AnalyticsEvent` data model, the `POST /api/events` write endpoint with its validation and server-side anonymization, and the injectable store that keeps it testable without a live database.
