...
- [x] 9.1 Done, 2026-08-14. Confirmed PR #47 merged into `main` first. Synced via a dedicated agent invoking `openspec-sync-specs`: both delta-declared requirements ("OpenGraph image generation is served from the build-time cache" — full replacement, now 3 scenarios; "No request-time filesystem reads of bundled application data" — description extended, only the "RAG index is loaded" scenario replaced) merged into `openspec/specs/cloudflare-deployment-compat/spec.md`. Verified via `git diff`: both untouched requirements ("The application builds and runs..." / "No Node.js-runtime Proxy or middleware") and the three unchanged scenarios under the second requirement are byte-identical, exactly as expected. **Noted, not fixed** (outside this delta's declared scope): the untouched "application builds and runs" requirement's own scenario still names the now-removed `/opengraph-image` route path — pre-existing staleness this change's delta never declared, flagged for a future follow-up rather than silently edited here. Archived this change to `openspec/changes/archive/2026-08-14-reduce-worker-bundle-size/` via `opsx:archive`
